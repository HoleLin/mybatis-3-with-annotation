...
public final class TypeHandlerRegistry {
  ...
  /**
   * 该集合记录了 JdbcType 与 TypeHandler 之间的关联关系。JdbcType 是一个枚举类型，
  ...
  private <T> TypeHandler<T> getTypeHandler(Type type, JdbcType jdbcType) {
    // 过滤掉ParamMap类型
    // ParamMap 是 MyBatis 框架中的一个特殊类，用于封装传递给 SQL 语句的参数。它通常用于在动态 SQL 中传递多个参数或者使用命名参数的情况下。
    // 由于 ParamMap 类型是一个特殊的临时对象，而不是真正的类型，因此它并不需要对应的 TypeHandle
